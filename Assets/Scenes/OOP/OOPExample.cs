...
// 클래스 : 객체 지향 프로그램의 기본 구성 요소이자 사용자 정의 데이터 타입
...
// 2. 상속 "class 자식클래스명 : 부모클래스명"의 형태로 설계합니다.
// 상황에 맞게 재정의하여 새로운 기능으로 만들어주는 것도 가능합니다.

// 3. 다형성 : 같은 이름의 변수, 함수여도 상황에 따라 다른 의미로 사용될 수 있음. 조건에 따라 다르게 수행되는 것.
#endregion
#region Data Structure
// 자료구조(Data Structure)
// 효율적이 접근과 수정을 가능하게 하는 자료의 조직, 관리, 저장, 데이터 값의 모임, 데이터의 관계 등을 종합해서 일컫는 말
...
// Stack<T>         : 데이터를 후입선출(LIFO)의 방식으로 사용할 경우 해당 자료구조를 사용합니다.
// Queue<T>         : 데이터를 선입선출(FIFO)의 방식으로 사용할 경우 해당 자료구조를 사용합니다.
...
//                    고정된 크기를 가지고 있고, 추가 소모되는 메모리 없이 크기에 맞게 제공됩니다.
//                    유니티 에디터 상에서는 배열과 리스트는 동일하게 취급되나 스크립트 작업 시에는 
//                    배열과 리스트의 사용법이 다르기 때문에 이에 유의합니다.

// 자료 구조 별 알고리즘의 단계 수 파악
...
//            데이터가 많아질 수록 작업의 효율이 나빠집니다.
// O(log n) : 아주 조금식 증가하는 형태의 곡선으로 데이터가 약 두 배 정도 증가할 때마다 1단계정도 늘어남.

// 효율 분석
// O(1) > O(log n) > O(n)

// O(1)은 O(log n)과 비교했을 때 데이터의 개수가 약 100개 이상을 넘을 경우 O(1)이 효율적이게 됩니다.
...
// Stack<T>             O(1)           -            O(1)            -
// Queue<T>             O(1)           -            O(1)            -
// Dictionary<K,V>      O(1)          O(1)          O(1)           인덱스 대신 키를 인덱스처럼 사용
