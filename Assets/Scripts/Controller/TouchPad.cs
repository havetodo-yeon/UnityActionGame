...
public class TouchPad : MonoBehaviour
{
    ...
    //터치 입력 중에 방향 컨트롤러의 영역안에 있는 입력을 구분하기 위한 고유 식별 코드 (아이디)
    ...
    private void FixedUpdate()  // 크로스 플랫폼
    {
        // 일반적인 경우에는 터치패드로 작업합니다.(모바일)
        HandleTouchInput();
    ...
    void HandleInput(Vector3 input)
    {
        ...
        if (_buttonPressed)
        {
            // 방향 컨트롤러의 기준 좌표부터 입력받은 좌표가 얼마나 떨어져있는 지를 계산
            Vector3 diffVector = (input - _startPos);

            // sqrMagnitude는 두 점 간의 거리의 제곱에 루트를 한 값
            // 비슷한 개념 Vector3.Distance (연산 속도가 느린 편), 대신 건축물 같은 정교한 값 구할 때 사용
            // sqrMagnitude는 단순하게 두 점 사이의 거리를 구할 때 사용
            // 정확한 거기를 체크하는 게 아닌 값의 크고 작은 지만 판단하는 식
