...
// Animator에 대한 요구. (반드시필요하다)
// 없을 경우 자동 추가
// 에디터에서 Animator 컴포넌트 제거할 수 없다.
// 만약 animator 컴포넌트 없다면 실행조차 되지 않는다
[RequireComponent(typeof(Animator))]
public class PlayerMovement : MonoBehaviour
{
    // 게임 오브젝트 내에 연결되어 있는 Animator 컴포넌트를 가져와ㅣ서 사용
        ...
        if (avatar)
        {
            ...
            if(rigidbody)
            {
                if(h != 0.0f && v != 0.0f)
                {
                    ...
                    // 해당 벡터 방향을 바라보는 회전 상태를 반환 하는 코드
                }
            }
        }
    }
    ...
    public void OnAttackDown()
    {
        ...
        // 코루틴은 반복 루틴에서 탈출하고 다시 그 시점으로 돌아오는 것이 가능하다.

        // StartCoroutine("StartAttack");   // 표기만 다르고 위와 같음
    }
    ...
    // return만 하면 바로 함수가 끝나는데 yield 덕분에 return 후 다시 돌아오는 형태가 됨
    ...
    IEnumerator StartAttack()
    {
        if(Time.time - lastAttackTime > 1.0f)
        {
            ...
            while(attacking)
            {
                ...
                yield return new WaitForSeconds(1.0f);  // 반환을 제공
            }
        }
    }
